...
hashtab hashtab_alloc( size_t max_array_size){
  int i;
  hashtab ht = (hashtab) malloc( sizeof( struct hashtab_struct));
  ht->array = (ht_bucket *) malloc( max_array_size*sizeof( ht_bucket));
  ht->max_array_size = max_array_size;
  ht->array_size = 0;
...
int hashtab_insert( hashtab ht, const char *key, const char *value){
  ...
  if( (double) ht->array_size / (double) ht->max_array_size > MAXFILL ){
    return HT_MEM_ERROR;
  }
